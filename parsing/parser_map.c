...
/*   Updated: 2023/02/27 10:24:35 by heloufra         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	valid_map_space(t_cub *cub, int i, int j)
{
	...
	if (j == 0 || i == 0 || i == len - 1 || j == ft_strlen(cub->map[i]) - 1)
		return (0);
	if (ft_isspace(cub->map[i - 1][j - 1]) || ft_isspace(cub->map[i - 1][j])
		|| ft_isspace(cub->map[i - 1][j + 1]) || ft_isspace(cub->map[i][j - 1])
		|| ft_isspace(cub->map[i][j + 1]) || ft_isspace(cub->map[i + 1][j - 1])
		|| ft_isspace(cub->map[i + 1][j]) || ft_isspace(cub->map[i + 1][j + 1]))
	{
		return (0);
	}
	return (1);
